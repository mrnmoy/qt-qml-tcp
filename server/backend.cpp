...
Backend::Backend(QObject *parent) : QObject(parent) {
  ...
  connect(server->tcpServer, &QTcpServer::newConnection, this,
          &Backend::smbConnectedToServer);
  connect(server, &Server::smbDisconnected, this,
...
QString Backend::startClicked() {
  if (!server->tcpServer->listen(QHostAddress::Any, 6547)) {
    ...
    connect(server->tcpServer, &QTcpServer::newConnection, server,
...
QString Backend::stopClicked() {
  if (server->tcpServer->isListening()) {
    disconnect(server->tcpServer, &QTcpServer::newConnection, server,
               &Server::newConnection);
...
void Backend::smbConnectedToServer() { emit smbConnected(); }

void Backend::smbDisconnectedFromServer() { emit smbDisconnected(); }

void Backend::gotNewMesssage(QString msg) { emit newMessage(msg); }
